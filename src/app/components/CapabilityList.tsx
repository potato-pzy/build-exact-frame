...
function CapabilityRow({
  ...
  return (
    <motion.article
      ...
      className={`w-full border-b border-gray-200 cursor-pointer select-none overflow-hidden group transition-colors duration-300 ${
        isOpen ? "bg-oec-grey-bg/60" : "bg-white hover:bg-oec-grey-bg/40"
      }`}
      onClick={onToggle}
    >
      {/* Mobile Row Layout (Matching Screenshot) */}
      <div className="lg:hidden p-5 flex items-start gap-4">
        <div className="w-12 h-12 rounded-xl bg-[#f8f9fa] border border-gray-100 flex items-center justify-center flex-shrink-0 mt-0.5">
          <Icon className="w-7 h-7" />
        </div>
        <div className="flex-1 min-w-0 pr-2">
          <h2
            className="font-serif-brand font-semibold text-oec-navy text-[16px] leading-[1.3] mb-1"
        ...
        <div className="flex-shrink-0 pt-1 text-gray-400">
          <svg className={`w-5 h-5 transform transition-transform duration-300 ${isOpen ? "rotate-90" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Desktop Row Layout */}
      <div className="hidden lg:grid max-w-[1280px] mx-auto pl-6 lg:pl-[67px] pr-6 lg:pr-0 grid-cols-[540px_1fr_170px] grid-rows-[auto_auto] items-stretch">
        <div className="lg:col-start-1 lg:row-start-1 pr-8 pt-9 lg:pt-12">
          <h2
            className="font-serif-brand font-light text-oec-navy text-[clamp(1.1rem,3.5vw,1.875rem)] leading-[1.24] tracking-[-0.02em] transition-colors duration-300 group-hover:text-oec-blue"
            style={{ fontVariationSettings: '"wdth" 100' }}
          >
            {capability.title}
          </h2>
        ...
        <div className="lg:col-start-1 lg:row-start-2 pr-8 mt-4 pb-9 lg:pb-12">
          <p className="font-sans-brand font-normal text-oec-body text-[15px] lg:text-[16px] tracking-[-0.015em] leading-[1.5]">
            {capability.eyebrow}
          </p>
        </div>
          ...
          <AnimatePresence initial={false}>
            {isOpen && (
              <motion.div
                ...
                transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
              >
                <p className="font-sans-brand font-light text-oec-body text-[15px] lg:text-[16px] tracking-[-0.015em] leading-[1.5]">
